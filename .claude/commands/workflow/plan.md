<!-- vendored from ~/.claude/shared/workflow/plan.md @ 2026-07-18
     customized: STANDARDS_DIR=docs/standard/
     local edits: none -->
Follow the Analysis & Planning workflow for the task described above.

Work through all 5 steps in sequence. Do not skip steps or jump to solutions.
(If the project keeps a canonical version, it is at docs/standard/ANALYSIS_AND_PLANNING.md.)
...
- Trace the relevant user action through the code (CLI command → core service (tailor / evaluate / apply) → store (SQLite + resume files))
- List which files/components/routes are involved
